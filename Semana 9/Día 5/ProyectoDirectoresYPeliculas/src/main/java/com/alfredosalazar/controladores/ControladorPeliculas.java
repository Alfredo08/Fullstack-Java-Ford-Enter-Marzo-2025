...
public class ControladorPeliculas {
	...
	public String peliculas(Model modelo, HttpSession sesion) {
		...
		for(int i = 0; i < listaDePeliculas.size(); i ++) {
			System.out.println(listaDePeliculas.get(0).getDirector().getId() + " " +
							   listaDePeliculas.get(0).getTitulo());
			
		}
		System.out.println("Id de director en sesion: " + sesion.getAttribute("id_director"));
	...
	public String agregarPelicula(@Valid @ModelAttribute("pelicula") Pelicula nuevaPelicula,
		...
		Long idDirector = (Long)sesion.getAttribute("id_director");
	...
	public String editarPelicula(@Valid @ModelAttribute("pelicula") Pelicula pelicula,
		...
		Pelicula peliculaActual = this.servicioPeliculas.obtenerPorId(peliculaId);
		Long idDirector = (long)sesion.getAttribute("id_director");
		Director directorActual = this.servicioDirectores.obtenerDirectorPorId(idDirector);
		// Al objeto del formulario de edición le asignamos ese ID
		pelicula.setId(peliculaActual.getId());
		pelicula.setDirector(directorActual);
